...
public class Util {
  ...
  public static boolean initAssignmentDb(DbHelper dbHelper, String ytdDomain) {
    ...
    if (json != null) {
      try {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
          ...
          String playlistId = assignmentJson.getString("playlistId");
          String instructions = assignmentJson.getString("loginInstruction");
          ...
          if (!dbHelper.hasAssignment(ytdDomain, id)) {
            Assignment assignment = new Assignment(ytdDomain, id);
            assignment.setCreated(created);
            assignment.setUpdated(updated);
            ...
            Log.d(MainActivity.TAG, assignment.toString());

            long id_ = dbHelper.insertAssignment(assignment);
            Log.d(LOG_TAG, "db insert id=" + id_);
          }
        }
      } catch (JSONException e) {
        e.printStackTrace();
